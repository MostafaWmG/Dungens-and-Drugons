...
/**
 * This is a TankFighterBuilder class for creating TankFighter
 * @author wmg
 */
public class TankFighterBuilder extends FighterBuilder{
	...
	public void setCharacterAbility() {
		...
		System.out.println("<<<Tank Fighter CREATED RANDOMLY>>> ");
		getFighter().setCharacterAbility();
		getFighter().replaceAbility(new Ability(AbilityEnum.CONSTITUTION,numbers.get(numbers.size()-1),(int)Math.floor( (numbers.get(numbers.size()-1) - 10) /2 )));
		getFighter().replaceAbility(new Ability(AbilityEnum.DEXTERITY,numbers.get(numbers.size()-2),(int)Math.floor( (numbers.get(numbers.size()-2) - 10) /2 )));
		getFighter().replaceAbility(new Ability(AbilityEnum.STRENGTH,numbers.get(numbers.size()-3),(int)Math.floor( (numbers.get(numbers.size()-3) - 10) /2 )));
